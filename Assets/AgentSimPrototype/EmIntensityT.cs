 ...
 * =======================================================================================================
 * Emotion Intensity ADT (EmIntensityT, EmIntensityChgT)
 * -------------------------------------------------------------------------------------------------------
...
namespace EMgine
{
    ...
    public sealed class EmIntensityT
    {
        ...
        public bool EqualsMinIntensity() => _intensity.CompareTo(_MIN_INTENSITY) == 0;
        public static EmIntensityT GetMinIntensity() => new EmIntensityT(_MIN_INTENSITY);

        public EmIntensityT Normalize(EmIntensityT scale)
        {
            EmIntensityT unit = null;

            if (scale.GetIntensity().Equals(0.0)) Utility.PrintMsg("E-EIT_INTENSITY_CANNOT_BE_ZERO", Array.Empty<string>());
        ...
        public EmIntensityT UpdateWithChg(EmIntensityChgT chg)
        {
            ...
            else if (chg.CompareToMinIntensity() < 0)
            {
                ...
                if (_intensity.CompareTo(absChg.ToReal()) > 0)
                {
        ...
        public override string ToString() => _intensity.ToString();

        /* Utility */
        private double GetIntensity() => _intensity;
        private static double GetMinIntensityAsValue() => _MIN_INTENSITY;
        private void SetIntensity(double i)
        {
             ...
             */
            if (i.CompareTo(_MIN_INTENSITY) < 0)
            {
                Utility.PrintMsg("W-EIT_INTENSITY_TOO_SMALL", new string[] { GetMinIntensity().ToString() });
        ...
        public sealed class EmIntensityChgT
        {
            ...
            */
            ...
            public int CompareToMinIntensity() => _iDelta.CompareTo(GetMinIntensityAsValue());

            public EmIntensityChgT ScaleByValue(double v) => new EmIntensityChgT(_iDelta * v);

            public double ToReal() => _iDelta;
            public override string ToString() => _iDelta.ToString();
