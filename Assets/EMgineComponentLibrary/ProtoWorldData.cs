 ...
 * ----------------------------------------------------------------------
 * These classes implement the WorldStateViewT, WorldEventT, 
 * DistBtwWorldStateViewsT, and DistChgBtwWorldStateViewsT abstract 
 * classes by defining a type for the S generic type inWorldStateViewT. 
 * Pretty much coppied from EMgineWorldData.cs with some variable name 
...
namespace AgentSimulation
{
    ...
    */
    ...
    public class AgentSimItemSlot : AgentSimWorldData, 
        IEquatable<AgentSimItemSlot>, IComparable<AgentSimItemSlot>
    {
        ...
        /// </summary>
        /// <remarks>
        /// Adding to a full slot (true + 1) and removing from 
        ///     an empty slot (false - 1) are both invalid.
        /// </remarks>
        ...
        /// Assumes already checked for invalid changes from <see 
        /// cref="AgentSimulation.ProtoWorldStateViewT.CheckSlotChanges(
        ///     AgentSimItemSlot[], int[])" />
        /// </summary>
        /// <param name="slotChange">The slot change.</param>
        /// <remarks>Copy pasted switch statement, 
        /// probably could be reduced <seealso cref="InvalidSlotChange(int)"/>.
        ///     </remarks>
        ...
        public AgentSimItemSlot SlotChange(int slotChange)
        {
            return (this.GetData(), slotChange) switch
            {
                // removing from a full slot
                (true, < 0) => new AgentSimItemSlot(false),
                // adding to an empty 
                (false, > 0) => new AgentSimItemSlot(true),
                // no state change, (_, 0)
                _ => new AgentSimItemSlot(this.GetData()), 
            };
        }


// nullable enable envoirnment:
...
#nullable disable
        ...
        // had to add this override for bool type because it kept
        // misdetecting types to compare to
        ...
        public bool Equals(AgentSimItemSlot other_slot)
        {
            ...
            // if two slots equal true
            if (_boolean == other_slot.GetData()) return true; 
