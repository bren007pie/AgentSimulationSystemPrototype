...
public class Test_ProtoIntegration
{
    ...
    static ProtoWorldStateViewT currentWorld = 
        new ProtoWorldStateViewT(new AgentSimItemSlot[] 
            { full_slot, empty_slot, empty_slot });
    ...
    public void Make_a_Goal()
    {
        ...
        ProtoDistanceBetweenWorldStates dist_1 = 
            (ProtoDistanceBetweenWorldStates)goal.Dist2Goal(goal_World); // 0
        ProtoWorldEventT remove_event = 
            new ProtoWorldEventT(new int[] { -1, -1, -1 }); // -3
        ProtoDistanceChangeCausedByEvent dist_2 = 
            (ProtoDistanceChangeCausedByEvent)
                goal.Dist2GoalChg(goal_World, remove_event); // -3

        Assert.That(dist_1.Equals(
            new ProtoDistanceBetweenWorldStates(goal_World, goal_World)));
    ...
    {
        // okay, in this example the intensity goes up
        // when we add something and make it closer to the goal
        ProtoWorldEventT event1 = 
            new ProtoWorldEventT(new int[] { 0, 1, 0 });

        ProtoDistanceChangeCausedByEvent dDelta1 = 
            (ProtoDistanceChangeCausedByEvent)
                currentWorld.CalcChangeCausedBy(event1);
        ...
        EmIntensityT joyIntensity = 
            new EmIntensityT(1.0d); 
        EmIntensityT joyIntensityPrime1 = 
            joyIntensity.UpdateWithChg(intensityDelta1);

        Assert.AreEqual(joyIntensityPrime1.ToReal(), 1.1d);

        // Joy only goes up, removing something only makes it further. 
        ProtoWorldEventT event2 =
            new ProtoWorldEventT(new int[] { -1, 0, 0 });
        ProtoDistanceChangeCausedByEvent dDelta2 = 
            (ProtoDistanceChangeCausedByEvent)
                currentWorld.CalcChangeCausedBy(event2);
        EmIntensityT.EmIntensityChgT intensityDelta2 = 
            EmIntensityCalcLib<AgentSimItemSlot>.CalcJoyIntensity(goal, 
                dDelta2);
        EmIntensityT joyIntensityPrime2 = 
            joyIntensity.UpdateWithChg(intensityDelta2);
    ...
    public void GenerateJoy()
    {
        ...
        ProtoWorldEventT event2 = 
            new ProtoWorldEventT(new int[] { 0, 1, 0 });
        ProtoWorldEventT event3 = 
            new ProtoWorldEventT(new int[] { 1, 1, 0 });
        ProtoWorldEventT event4 = 
            new ProtoWorldEventT(new int[] { 1, 1, 1 });
        /// < param name = "epsilonJ" > Threshold value, 0 distance.</param>
        /// 0 distance threshold
        ProtoDistanceChangeCausedByEvent epsilonJ = 
            new ProtoDistanceChangeCausedByEvent(event1); 
        ...
        (WorldStateViewT<AgentSimItemSlot>.DistBtwWorldStateViewsT 
            distPrev, WorldStateViewT<AgentSimItemSlot>.DistBtwWorldStateViewsT 
            distNow, WorldStateViewT<AgentSimItemSlot>.DistChgBtwWorldStateViewsT 
                distDelta)? joy1;
        (WorldStateViewT<AgentSimItemSlot>.DistBtwWorldStateViewsT distPrev, 
        ...
        joy1 = EmGenFunLib<AgentSimItemSlot>.GenerateJoy(goal, 
            noItems, event1, epsilonJ);
        joy2 = EmGenFunLib<AgentSimItemSlot>.GenerateJoy(goal, 
            noItems, event2, epsilonJ);
        joy3 = EmGenFunLib<AgentSimItemSlot>.GenerateJoy(goal, 
            noItems, event3, epsilonJ);
        joy4 = EmGenFunLib<AgentSimItemSlot>.GenerateJoy(goal, 
            noItems, event4, epsilonJ);

        // the way to make this into a loop (without a nullable value)
        // is to overwrite the value of joy
        ...
        if (joy2 != null)  // if passes all the threshold conditions
        {
            ...
            (previousDistance, newDistance, distanceChange) = 
                joy3 ?? default;
            EmIntensityT.EmIntensityChgT intensityDelta3 = 
                EmIntensityCalcLib<AgentSimItemSlot>.CalcJoyIntensity(goal,
                    distanceChange);
            // update the value if changed
            joyIntensity3Prime = 
                joyIntensity.UpdateWithChg(intensityDelta3); 

            (previousDistance, newDistance, distanceChange) = joy4 ?? default;
            EmIntensityT.EmIntensityChgT intensityDelta4 = 
                EmIntensityCalcLib<AgentSimItemSlot>.CalcJoyIntensity(goal, 
                distanceChange);
            // update the value if changed
            joyIntensity4Prime = joyIntensity.UpdateWithChg(intensityDelta4);
